...
namespace gfx {
	...
	vector<io::InputEvent> generateInputEvents();
	...
	class Font : public RefCounter {
	  public:
		...
		FRect draw(FRect const &rect, FontStyle const &style, char const *text) const;
		FRect draw(FRect const &rect, FontStyle const &style, const TextFormatter &fmt) const {
			return draw(rect, style, fmt.text());
		}
		FRect draw(FRect const &rect, FontStyle const &style, string const &str) const {
			return draw(rect, style, str.c_str());
		}

		FRect draw(float2 const &pos, FontStyle const &style, char const *text) const {
			return draw(FRect(pos, pos), style, text);
		}
		FRect draw(float2 const &pos, FontStyle const &style, TextFormatter const &fmt) const {
			return draw(FRect(pos, pos), style, fmt.text());
		}
		FRect draw(float2 const &pos, FontStyle const &style, string const &str) const {
	  ...
	  private:
		...
		int genQuads(char const *str, float2 *out_pos, float2 *out_uv, int buffer_size) const;

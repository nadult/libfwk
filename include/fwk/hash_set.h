...
template <class TKey> class HashSet {
  public:
	using Key = TKey;
	using HashValue = u32;
  ...
  private:
	...
	template <typename TNodePtr, typename TPtr, typename TRef> class NodeIterator {
		friend class HashSet;

	  public:
		typedef std::forward_iterator_tag iterator_category;

		explicit NodeIterator(TNodePtr node, const HashSet *map) : m_node(node), m_map(map) {}
		template <typename UNodePtr, typename UPtr, typename URef>
		NodeIterator(const NodeIterator<UNodePtr, UPtr, URef> &rhs)
		...
		bool operator==(const NodeIterator &rhs) const { return rhs.m_node == m_node; }
		bool operator!=(const NodeIterator &rhs) const { return !(rhs == *this); }
  ...
  public:
	using iterator = NodeIterator<Node *, Key *, Key &>;
	using const_iterator = NodeIterator<const Node *, const Key *, const Key &>;
	static constexpr int initial_capacity = 64;
	static_assert(isPowerOfTwo(initial_capacity));

	// TODO: move constructors, etc.
	HashSet() {}
	explicit HashSet(int initial_bucket_count) { reserve(initial_bucket_count); }
	HashSet(const HashSet &rhs) { *this = rhs; }
	~HashSet() { deleteNodes(); }

	// Load factor controls hash map load. 4 means 100% load, ie. HashSet will grow
	...
	Pair<iterator, bool> emplace(const Key &key) {
		...
		Node *n = find_for_insert(key, hash);
		if(n->isOccupied()) {
			DASSERT(hash == n->hash && key == n->data);
	...
	vector<Key> values() const { return keys(); }
...
template <typename TKey> typename HashSet<TKey>::Node HashSet<TKey>::s_empty_node;
}

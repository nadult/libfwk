...
namespace io {
	...
	InputEvent::InputEvent(Type key_type, int key, int iter)
		: m_mouse_pos(0.0f, 0.0f), m_key(key), m_iteration(iter), m_type(key_type) {
		DASSERT(isKeyEvent());
	}

	InputEvent::InputEvent(Type mouse_type, int key, const int2 &mouse_move)
		: m_mouse_pos(0.0f, 0.0f), m_mouse_move(mouse_move), m_key(key), m_type(mouse_type) {
	...
	bool InputEvent::keyDownAuto(int key, int period) const {
		DASSERT(period >= 1);
		return m_key == key && m_type == key_down_auto &&
			   (period == 1 || m_iteration % period == 0);
	}
	...
	bool InputEvent::mouseKeyUp(int key) const { return m_type == mouse_key_up && m_key == key; }

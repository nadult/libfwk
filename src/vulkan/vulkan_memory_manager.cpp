...
VulkanMemoryManager::VulkanMemoryManager(VDeviceRef device)
	...
	auto &info = VulkanInstance::ref()->info(device->physId());
...
void VulkanMemoryManager::startFrame(int frame_idx) {
	m_frame_index = frame_idx;
	m_frames[frame_idx].offset = 0;
}
...
void VulkanMemoryManager::free(AllocId ident) {
	switch(ident.type) {
...
auto VulkanMemoryManager::frameAlloc(u64 size, uint alignment) -> Ex<Allocation> {
	...
	if(aligned_offset + size > frame.size) {
		...
		u64 new_size = max(frame.size * 2, m_frame_allocator_base_size, aligned_offset + size);
		auto new_mem = EX_PASS(m_device->allocDeviceMemory(new_size, type_index));
		frame.size = new_mem->size();
		frame.memory = new_mem;
		frame.offset = 0;
		aligned_offset = 0;
	}
	...
	return Allocation{alloc_id, frame.memory, aligned_offset};
}

Ex<void> VulkanMemoryManager::frameAlloc(PVBuffer buffer) {
	DASSERT("Run startFrame first" && m_frame_index != -1);
	auto buf_handle = buffer.handle();
	VkMemoryRequirements mem_requirements;
	vkGetBufferMemoryRequirements(m_device_handle, buf_handle, &mem_requirements);
	auto type_index = m_domains[m_frame_allocator_domain].type_index;
	if(!(mem_requirements.memoryTypeBits & (1u << type_index)))
		return ERROR("VulkanBuffer requires different memory type (index:% not in mask:%)",
					 type_index, stdFormat("%x", mem_requirements.memoryTypeBits));

	auto allocation = EX_PASS(frameAlloc(mem_requirements.size, mem_requirements.alignment));
	if(vkBindBufferMemory(m_device_handle, buffer, allocation.mem_handle, allocation.mem_offset) !=
	   VK_SUCCESS)
